...
from eventHist import * # Contains plot_charge function (still needed for plot_charge within get_charges_of_these_events)
from plotting_functions import * # Contains plot_fpga_time_delta (though not explicitly used for plotting here)

# Ensure read_hdffile is defined, if not, import from HDFWriterModuleInspection
try:
    read_hdffile
except NameError:
...
plotting_map = [6,11,7,12,8,13,9,14,1,16,2,17,3,18,4,19,5,20] # Original mapping for reference
...
def event_matching(timestamp_ref_ns, timestamp_ns, window_ns=100.):
    ...
    while ptr_ref < len(timestamp_ref_ns) and ptr_target < len(timestamp_ns):
        ...
        # If the absolute difference is within the window, consider it a match
...
def get_waveform_at_this_timestamp(filename, timestamp):
    ...
    Returns:
        tuple: A tuple containing:
            ...
            - np.array: `wf_ch1`, ADC counts for channel 1.
            - np.array: `wf_ch2`, ADC counts for channel 2.
    ...
    if len(matching_indices) > 0:
        iev = matching_indices[0] # Take the first match if duplicates exist
        n = nsamples[iev] # Number of samples for this event
        x = np.array([ i * (1e9/60e6) for i in range(n) ]) # Convert sample index to time in nanoseconds
        wf_ch1 = np.array(adc_ch1[iev][:n]) # Slice to the actual number of samples
        wf_ch2 = np.array(adc_ch2[iev][:n]) # Slice to the actual number of samples
...
def get_charges_of_these_events(filename, evidx_list): 
    ...
    for iev in evidx_list: # Iterate through provided event indices directly
        if iev < len(q_ch1_pC): # Ensure index is valid for existing data
            ret_charges_ch1.append(q_ch1_pC[iev])
            ret_charges_ch2.append(q_ch2_pC[iev])
        else:
...
# Function to plot a multi-subplot comparison for waveforms
def plot_multi_pmt_waveforms_comparison(pmt_wf_data_collection, plot_title, output_filename, output_dir):
    ...
    # แถว 1 : [pmt08, pmt 10, pmt 12, pmt 14]
    # แถว 2 : [pmt 02, pmt 06, pmt08, pmt 10] (หมายเหตุ: PMT 8 และ 10 ซ้ำกัน)
    # แถว 3 : [pmt 01, pmt03, pmt05, pmt07]
    # แถว 4 : [pmt 09, pmt 11, pmt 13, pmt 15]
    custom_subplot_order_for_4x4_grid = np.array([
        8, 10, 12, 14,
        2,  6,  8, 10, # Note: PMT 8 and 10 are repeated here
        1,  3,  5,  7,
        9, 11, 13, 15
    ])
    ...
    global_ymin_adc = 0
    global_ymax_adc = 4096 # Assuming a 12-bit ADC, typical range is 0-4095
    ...
    for pmt_id in pmt_wf_data_collection.keys():
        ...
        if data and data['x_wf'] is not None and len(data['x_wf']) > 1:
    ...
    # This loop ensures cleanup if the custom_subplot_order_for_4x4_grid had fewer than 16 elements,
    # or if some unexpected scenario occurs.
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", "-o", help="Output directory for plots (default: ./plots_output)", type=str, default='./plots_output') # New default output directory
    ...
    for fname in infilenames:
        ...
        path_parts = os.path.split(fname) # Corrected: os.path.split
        
        # Robust parsing for LOM name (Location of Module)
        # Attempt to extract LOM name from parent directory.
        # This heuristic tries to identify if the immediate parent directory is a 'data' or 'run' folder,
        # and if so, it looks one level up for the actual LOM name.
        if len(path_parts) >= 2:
            this_lom_candidate = os.path.basename(path_parts[-2]) # Corrected: Added os.path.basename to get just the directory name
            if 'data' in this_lom_candidate.lower() or 'run' in this_lom_candidate.lower():
                parent_dir_of_candidate = os.path.basename(os.path.dirname(fname)) # Corrected: os.path.basename, os.path.dirname
        ...
        file_base = os.path.basename(fname) # Corrected: os.path.basename
    ...
    for pmt_id in pmt_ids_with_data:
        ...
        # Robustly handle missing "FPGAtime" key by providing an empty list as fallback
    ...
    for i_pmt in pmt_ids_with_data:
        for j_pmt in pmt_ids_with_data:
            ...
            # Use event_matching to get the count of coincident events
    ...
    # User-provided order for X axis: [14,12,10,8,6,4,2,0,1,3,5,7,9,11,13,15] (left to right)
    # User-provided order for Y axis: [14,12,10,8,6,4,2,0,1,3,5,7,9,11,13,15] (bottom to top)
    #
    # The order for labels, assuming top-to-bottom reading for Y axis as in the reference image.
    ...
    lom18_y_indices_filtered = [p for p in lom18_y_labels_order if p in pmt_ids_with_data] # These are the actual PMT IDs to use for indexing

    # Reorder the coincidence matrices based on the LOM18 physical order.
    # np.ix_ is used to correctly index both rows (Y-axis) and columns (X-axis) simultaneously for the reordering.
    ...
    # Changed edgecolors to white, increased linewidth for clearer grid.
    ...
    # Ticks are placed at the center of each cell (+0.5)
    ...
    # The split point is based on PMT ID 0. We need its index in the filtered lists.
    # For lom18_display_order_xy = [14,12,10,8,6,4,2,0,1,3,5,7,9,11,13,15]
    # PMT 0 is at index 7. So the vertical line should be after index 7.5.
    # PMT 0 is at index 7. So the horizontal line should be after index 7.5.
    ...
    try:
        ...
        # Vertical line: after PMT 0 on the X-axis (at coordinate index + 0.5)
        ax_matrix.axvline(split_idx_x + 0.5, color='black', linestyle='-', linewidth=2) 
        # Horizontal line: after PMT 0 on the Y-axis (at coordinate index + 0.5)
        ax_matrix.axhline(split_idx_y + 0.5, color='black', linestyle='-', linewidth=2)
        
        # Add "← upper" and "lower →" labels as text annotations
        # These positions are adjusted using `transform=ax_matrix.transAxes` for relative positioning.
        # `xycoords=(x_frac, y_frac)`: fraction of the axes from 0 to 1.
        # `rotation`: 0 for horizontal, 90 for vertical.

        # Label "← upper" for the Upper Hemisphere on the X-axis (left part)
        ax_matrix.text(
            0.5 * (split_idx_x + 0.5) / len(lom18_x_indices_filtered), # X-position (normalized center of left segment)
            -0.08, # Y-position (below X-axis, adjusted for tight_layout)
            '← upper', 
            color='black', fontsize=10, ha='center', va='top', 
            transform=ax_matrix.transAxes # Use axes fraction for position
        )
        # Label "lower →" for the Lower Hemisphere on the X-axis (right part)
        ax_matrix.text(
            ( (split_idx_x + 0.5) + (len(lom18_x_indices_filtered) - (split_idx_x + 0.5)) ) / 2 / len(lom18_x_indices_filtered), # X-position (normalized center of right segment)
            -0.08, # Y-position (below X-axis, adjusted for tight_layout)
            'lower →', 
            color='black', fontsize=10, ha='center', va='top', 
            transform=ax_matrix.transAxes # Use axes fraction for position
        ...
        # Label "← upper" for the Upper Hemisphere on the Y-axis (top part)
        ax_matrix.text(
            -0.08, # X-position (left of Y-axis, adjusted for tight_layout)
            ( (split_idx_y + 0.5) + (len(lom18_y_indices_filtered) - (split_idx_y + 0.5)) ) / 2 / len(lom18_y_indices_filtered), # Y-position (normalized center of upper segment, inverted due to invert_yaxis)
            '← upper', 
            color='black', fontsize=10, ha='right', va='center', rotation=90, 
            transform=ax_matrix.transAxes # Use axes fraction for position
        )
        # Label "lower →" for the Lower Hemisphere on the Y-axis (bottom part)
        ax_matrix.text(
            -0.08, # X-position (left of Y-axis, adjusted for tight_layout)
            (split_idx_y + 0.5) / 2 / len(lom18_y_indices_filtered), # Y-position (normalized center of lower segment, inverted)
            'lower →', 
            color='black', fontsize=10, ha='right', va='center', rotation=90, 
            transform=ax_matrix.transAxes # Use axes fraction for position
        )
