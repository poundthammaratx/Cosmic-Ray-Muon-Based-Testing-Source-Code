...
# --- Start: Helper Functions (from muon_analysis_coincidence_script.py) ---
...
def plot_all_charge_dists_pe(filenames, assumed_gain=5e6, save_suffix='pe', output_dir='./figs'):
    ...
    for infile in filenames:
        ...
        ax_ch1.set_xlim([xlims_pe_narrow[0],xlims_pe_narrow[1]]) # Use PE-converted wide xlims
        ax_ch1.grid('xy', linestyle=':', lw=0.5)
        ax_ch1.tick_params(labelsize=6)
        ax_ch1.set_yscale('log')
        if subplot_index in [5, 17,18,19,20]:
...
if __name__ == "__main__":
    ...
    # --- NEW ADDITION: Prepare for Waveform Plotting - Find Coincident Events ---
    print("\n--- Preparing Coincident Event Data for Waveform Plotting ---")
    anchor_pmt_for_waveform_plot = 8 # As defined in your script. You can change this.
    target_pmts_ids = [p for p in pmt_ids_with_data if p != anchor_pmt_for_waveform_plot] # All other PMTs with data

    # Get the timestamps for the anchor PMT
    anchor_timestamps_for_wf_plot = all_pmt_event_times.get(anchor_pmt_for_waveform_plot)

    match_results_all_targets = [] # This will store the coincident timestamps for each target PMT relative to the anchor
    
    # Only proceed if the anchor PMT has data
    if anchor_timestamps_for_wf_plot is not None and anchor_timestamps_for_wf_plot.size > 0:
        # For each target PMT, find coincidences with the anchor PMT
        for target_pmt_id in target_pmts_ids:
            target_timestamps = all_pmt_event_times.get(target_pmt_id)
            if target_timestamps is not None and target_timestamps.size > 0:
                # event_matching returns list of [t_ref, t_target] for matched events
                # We want to store the t_target for each anchor event if a match exists.
                matched_pairs, _ = event_matching(anchor_timestamps_for_wf_plot, target_timestamps, window_ns=coincidence_window_ns)
                
                # Create a list where the index corresponds to the anchor event's original index
                # and the value is the coincident timestamp from the target PMT (or None if no match)
                matched_target_timestamps_for_this_pmt = [None] * len(anchor_timestamps_for_wf_plot)
                
                # Build a dictionary for faster lookup of target timestamps by anchor timestamp
                matched_dict = {ref_ts: target_ts for ref_ts, target_ts in matched_pairs}

                for i, anchor_ts_val in enumerate(anchor_timestamps_for_wf_plot):
                    # Check if this anchor_ts_val was part of a matched pair
                    if anchor_ts_val in matched_dict:
                        matched_target_timestamps_for_this_pmt[i] = [anchor_ts_val, matched_dict[anchor_ts_val]] # Store as [anchor_ts, target_ts]
                
                match_results_all_targets.append(matched_target_timestamps_for_this_pmt)
            else:
                # If target PMT has no data, add an empty list or None for it to maintain alignment
                match_results_all_targets.append([None] * len(anchor_timestamps_for_wf_plot))
    else:
        print("Warning: Anchor PMT has no events. Cannot proceed with waveform plotting.")
        match_results_all_targets = [] # Ensure it's defined but empty if no anchor data.

    # --- End NEW ADDITION ---

    # --- Start: Waveforms of Coincidence Events (from muon_analysis_coincidence_script.py) ---
    print("\n--- Plotting Coincident Event Waveforms ---")
    num_waveforms_to_plot = 100 
    waveforms_plotted_count = 100
    ...
    # Ensure anchor_timestamps_for_wf_plot is a NumPy array and not empty before sorting/iterating
    if not (isinstance(anchor_timestamps_for_wf_plot, np.ndarray) and anchor_timestamps_for_wf_plot.size > 0):
        print(f"Warning: No valid anchor timestamps found for plotting waveforms (PMT {anchor_pmt_for_waveform_plot}). This might be due to charge_cut_coincidence being too strict or no events in anchor PMT.")
        print("Skipping waveform plotting.")
    else:
        # Create a dictionary to map `target_pmt_ids` to the indices in `match_results_all_targets`.
        target_pmt_id_to_match_results_idx = {pmt_id: i for i, pmt_id in enumerate(target_pmts_ids)}

        # Iterate through each event of the anchor PMT
        for iev_anchor, anchor_ts in enumerate(anchor_timestamps_for_wf_plot): # Use unsorted original array
            if waveforms_plotted_count >= num_waveforms_to_plot:
                break

            is_multi_pmt_coincidence = False
            active_pmts_in_event = [] 
            
            # Check if this anchor event has a coincidence with *any* target PMT
            for pmt_id in target_pmts_ids:
                # Get the relevant list from match_results_all_targets
                match_results_list_for_this_target_pmt = match_results_all_targets[target_pmt_id_to_match_results_idx[pmt_id]]
                
                # Check if there's a coincident hit for this anchor event (iev_anchor) with the current target pmt_id
                if iev_anchor < len(match_results_list_for_this_target_pmt) and \
                   match_results_list_for_this_target_pmt[iev_anchor] is not None and \
                   match_results_list_for_this_target_pmt[iev_anchor][1] is not None:
                    
                    is_multi_pmt_coincidence = True
                    active_pmts_in_event.append(pmt_id)

            if not is_multi_pmt_coincidence:
                continue # Skip if this anchor event is not coincident with any other PMT
            ...
            # Iterate through all PMT IDs (0-19 based on plotting_map) for plotting
            # We need to ensure we plot all 20 subplots, even if no data or no coincidence
            for pmt_id_to_plot in range(len(plotting_map)): 
                if pmt_id_to_plot in plotted_pmts_in_this_event_fig: # Should not happen with this loop structure but good to have
                    continue

                subplot_idx = plotting_map[pmt_id_to_plot]
                ax_wf = fig_all_wf.add_subplot(4,5, subplot_idx)
                
                pmt_infile = pmt_to_infile_map.get(pmt_id_to_plot)
                
                coincident_ts_for_pmt = None
                # If the current PMT to plot is our anchor PMT, use its timestamp
                if pmt_id_to_plot == anchor_pmt_for_waveform_plot:
                    coincident_ts_for_pmt = anchor_ts # Use the current anchor event's timestamp
                elif pmt_id_to_plot in target_pmts_ids:
                    # Look up the coincident timestamp for this target PMT
                    target_pmt_idx_in_list = target_pmt_id_to_match_results_idx[pmt_id_to_plot]
                    if iev_anchor < len(match_results_all_targets[target_pmt_idx_in_list]) and \
                       match_results_all_targets[target_pmt_idx_in_list][iev_anchor] is not None:
                ...
                if pmt_infile is None:
                    ...
                    ax_wf.set_title(f"pmt{pmt_id_to_plot}", fontsize=8) 
                elif coincident_ts_for_pmt is not None:
                    x, wf1, wf2 = get_waveform_at_this_timestamp(pmt_infile, coincident_ts_for_pmt)
                    if len(wf1) > 0:
                        ax_wf.plot(x, wf1, lw=0.5, label=f"pmt{pmt_id_to_plot}/\n@{coincident_ts_for_pmt}")
                        ax_wf.set_title(f"pmt{pmt_id_to_plot}", fontsize=8)
                    else: 
                ...
                plotted_pmts_in_this_event_fig.add(pmt_id_to_plot)
    ...
    # Re-initialize and populate all_charge_hists_for_agg_plot for this specific plot
    all_charge_hists_for_agg_plot = [[] for _ in range(len(plotting_map))]
    for pmt in range(len(plotting_map)):
        infile_for_pmt = pmt_to_infile_map.get(pmt)
        if infile_for_pmt is None:
            continue
        data = read_hdffile(infile_for_pmt, quiet=True)
        # Use a wider range for this overall plot, consistent with wide-range in plot_all_charge_dists
        ch1_agg, _ = plot_charge(data, no_plot=True, xmin_ch1=-1, xmax_ch1=200, nbin_ch1=114*4,)
        all_charge_hists_for_agg_plot[pmt].append([ch1_agg[0], ch1_agg[1]])


    fig_charge_agg = plt.figure(figsize=(15, 12)) 
    for pmt in range(len(plotting_map)): 
        if pmt >= len(all_charge_hists_for_agg_plot) or not all_charge_hists_for_agg_plot[pmt]: 
            continue

        if len(all_charge_hists_for_agg_plot[pmt]) > 0: 
            x_sum_combined = all_charge_hists_for_agg_plot[pmt][0][0] 
            y_sum_combined = np.zeros_like(x_sum_combined, dtype=float)
            for x_data, y_data in all_charge_hists_for_agg_plot[pmt]:
    ...
    output_dir_debug_dists = args.output_dir 
