# analysis_quick_check.py (Revised for PE Conversion, Logarithmic Y-axis, and Settable Output Directory)
#
# - Reads HDF files.
# - Plots essential information.
# - Plots Charge Distributions in pC and Photoelectrons (PE) with logarithmic Y-axis for better visibility of higher charge events.
# - Allows specifying an output directory for saved plots.
#
import h5py
import numpy as np
import datetime
...
# === START: Path and Correct Import Management Section ===
# Get the path of the current script (analysis_quick_check.py)
script_dir = os.path.dirname(os.path.abspath(__file__))
# Create the path to the 'util' folder
...
# === END: Path and Correct Import Management Section ===
...
def plot_all_charge_dists(filenames, xlims, unit_label='Charge (pC)', save_suffix='ch1', output_dir='./figs'): # <<< Added output_dir here
    """
    ...
    for infile in filenames:
        ...
        ax_ch1.set_yscale('log') # <<<--- Added log scale here
        ...
        ax_ch1_wide.set_yscale('log') # <<<--- Added log scale here
        if subplot_index in [5, 17,18,19,20]:
    ...
    if not os.path.exists(output_dir): # <<< Using output_dir here
        os.makedirs(output_dir) # <<< Using output_dir here

    fig_ch1.subplots_adjust(wspace=0, hspace=0)
    fig_ch1.savefig(f'{output_dir}/all_charge_{save_suffix}.pdf') # <<< Using output_dir here
    fig_ch2.subplots_adjust(wspace=0, hspace=0)
    fig_ch2.savefig(f'{output_dir}/all_charge_{save_suffix}_ch2.pdf') # <<< Using output_dir here
    fig_ch1_wide.subplots_adjust(wspace=0, hspace=0)
    fig_ch1_wide.savefig(f'{output_dir}/all_charge_{save_suffix}_wide.pdf') # <<< Using output_dir here
    plt.close(fig_ch1)
    plt.close(fig_ch2)
...
def plot_all_charge_dists_pe(filenames, assumed_gain=5e6, save_suffix='pe', output_dir='./figs'): # <<< Added output_dir here
    """
...
def plot_all_fpgatime(filenames, output_dir='./figs'): # <<< Added output_dir
    """
    Plots FPGA time distributions.
    Files are saved to the specified output_dir.
    """
    ...
    outname=f'{output_dir}/all_fpgatime.pdf' # <<< Using output_dir here
...
def plot_all_dt(filenames, output_dir='./figs'): # <<< Added output_dir
    """
    Plots delta time distributions.
    Files are saved to the specified output_dir.
    """
    fig = plt.figure(figsize=(12, 9))
    for infile in filenames:
    ...
    outname=f'{output_dir}/all_dt.pdf' # <<< Using output_dir here
    fig.subplots_adjust(wspace=0, hspace=0)
    fig.savefig(outname)
    print(f'Saved {outname}')
    plt.close(fig)
...
def plot_all_wf(filenames, output_dir='./figs'): # <<< Added output_dir
    """
    Plots waveforms.
    Files are saved to the specified output_dir.
    """
    ...
    fig.subplots_adjust(wspace=0, hspace=0)
    fig_ch2.subplots_adjust(wspace=0, hspace=0)
    outname=f'{output_dir}/all_wf.pdf' # <<< Using output_dir here
    fig.savefig(outname)
    print(f'Saved {outname}')
    outname=f'{output_dir}/all_wf_ch2.pdf' # <<< Using output_dir here
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", "-o", help="Output directory for plots (default: ./figs)", type=str, default='./figs')
    ...
    # Changed save_suffix from 'muon_pC' to 'spe_pC' or 'charge_pC'
    plot_all_charge_dists(filenames, [-0.1, 1.5], unit_label='Charge (pC)', save_suffix='spe_pC', output_dir=args.output_dir)
    ...
    # Changed save_suffix from 'muon_pe' to 'spe_pe' or 'charge_pe'
    plot_all_charge_dists_pe(filenames, assumed_gain=5e6, save_suffix='spe_pe', output_dir=args.output_dir)

    # Original other plots (FPGAtime, dt, waveforms)
    # To enable these functions and save them in the same output_dir
    # plot_all_fpgatime(filenames, output_dir=args.output_dir)
    # plot_all_dt(filenames, output_dir=args.output_dir)
    # plot_all_wf(filenames, output_dir=args.output_dir)
