...
def plot_single_charge_correlation_subplot(ax, all_events_df, lom_name, m_slope, c_intercept, r_squared, fit_q1_min_pC, fit_q1_max_pC, plot_type='log', cbar_vmin=None, cbar_vmax=None):
    ...
    if plot_type == 'log':
        # Ensure vmin and vmax are set for LogNorm for consistent color scale mapping
...
def plot_charge_correlation_per_lom(df_all_events_lom, lom_name, output_dir, lom_output_prefix, fit_q1_min_pC, fit_q1_max_pC):
    """
    Plots Ch1 vs Ch2 charge correlation for a single LOM, with linear and log scales side-by-side,
    and fixed colorbar ranges. Saves to a single PDF file per LOM.
    """
    ...
    df_fit = df_all_events_lom[(df_all_events_lom['ch1_pC'] >= fit_q1_min_pC) & (df_all_events_lom['ch1_pC'] <= fit_q1_max_pC)]
    ...
    if len(df_fit) >= 2:
        try:
            ...
            warnings.warn(f"Could not fit linear regression for LOM {lom_name}.")
    else:
        warnings.warn(f"Not enough valid charge data points in fit range [{fit_q1_min_pC:.1f}, {fit_q1_max_pC:.1f}] pC for LOM {lom_name}. Skipping regression.")

    # Plot Linear Scale
    mappable_linear = plot_single_charge_correlation_subplot(
        axes[0], df_all_events_lom, lom_name,
        m_slope, c_intercept, r_squared, fit_q1_min_pC, fit_q1_max_pC, plot_type='linear',
        cbar_vmin=LINEAR_CBAR_VMIN, cbar_vmax=LINEAR_CBAR_VMAX
    )
    cbar_linear = fig.colorbar(mappable_linear, ax=axes[0], label='Number of Events (Linear Scale)')


    # Plot Log Scale
    mappable_log = plot_single_charge_correlation_subplot(
        axes[1], df_all_events_lom, lom_name,
        m_slope, c_intercept, r_squared, fit_q1_min_pC, fit_q1_max_pC, plot_type='log',
    ...
    cbar_log = fig.colorbar(mappable_log, ax=axes[1], label='Number of Events (Log Scale)', format='%.0e', ticks=log_ticks)

    fig.suptitle(f"Charge Correlation (Ch1 vs Ch2) for {lom_name}", fontsize=14, y=1.02)
    ...
    plt.savefig(os.path.join(output_dir, output_filename))
    plt.close(fig)
    print(f"Saved Charge Correlation plot for {lom_name} to: {os.path.join(output_dir, output_filename)}")
...
if __name__ == "__main__":
    ...
    # --- Prepare combined Coincidence Matrices figures ---
    ...
    matrices_linear_mappables = [] # To collect mappables for common colorbar
    ...
    matrices_log_mappables = [] # To collect mappables for common colorbar
    ...
    for lom_idx, current_lom_folder_path in enumerate(sorted_lom_data_dirs):
        ...
        # --- Data preparation for Charge Correlation for current LOM ---
        ...
        df_all_events_current_lom = pd.DataFrame(all_events_data_current_lom)
        df_all_events_current_lom = df_all_events_current_lom.dropna(subset=['ch1_pC', 'ch2_pC'])
        df_all_events_current_lom = df_all_events_current_lom[(df_all_events_current_lom['ch1_pC'] > 1) & (df_all_events_current_lom['ch2_pC'] > 1)]

        # --- MODIFIED: Call new function to plot Charge Correlation for the current LOM in a separate file ---
        if not df_all_events_current_lom.empty:
            plot_charge_correlation_per_lom(
                df_all_events_current_lom,
                current_lom_name,
            ...
            print(f"No valid events to plot Charge Correlation for {current_lom_name}. Skipping.")
        ...
        # Note: These plots are per-LOM as generated inside the loop, not combined across LOMs.
        # The variables fig_matrices_all_loms_linear and fig_matrices_all_loms_log
        # are for the *combined* matrices plot at the very end of the script.
        # This part here creates separate matrix plots per LOM if needed.
        # Based on the image, the user likely wants the *combined* view as the primary one.
        # The following 'fig_matrices' and 'fig_rates_total' are for individual LOM plots.
        # I'll retain them but clarify their purpose.

        # Individual LOM Coincidence Matrices/Rates plots (separate files for each LOM and threshold)
        # This part of the code implicitly creates separate figures and saves them per LOM.
        ...
        if not lom18_x_indices_filtered or not lom18_y_indices_filtered:
            ...
            # Loop for Coincidence Matrices (for individual LOM plots, not the combined ones)
            for c_idx, current_threshold_pC in enumerate(COINCIDENCE_THRESHOLDS_PC): # Iterate over thresholds
                ...
                # This part plots into the *overall* combined figures defined before the LOM loop
                ...
                # These are separate rate plots saved per LOM, not part of the combined matrices figure.
    ...
    # --- Finalize and save all combined Coincidence Matrices plots ---
